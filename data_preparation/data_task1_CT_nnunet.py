...
def collect_ct_files(data_root):
    ...
    for task_dir in task_dirs:
        train_image_dir = task_dir / "train" / "labeled" / "images"
        train_mask_dir = task_dir / "train" / "labeled" / "labels"

        train_unlabeled_image_dir = task_dir / "train" / "unlabeled"
        test_unlabeled_image_dir = task_dir / "val" / "images"

        for p in sorted(train_image_dir.glob("*.nii.gz")):
            rows.append(
                {
                    "split": "TrL",
                    "file_type": "image",
        ...
        for p in sorted(train_mask_dir.glob("*-seg.nii.gz")):
            rows.append(
                {
                    "split": "TrL",
                    "file_type": "mask",
        ...
        for p in sorted(train_unlabeled_image_dir.glob("*.nii.gz")):
            rows.append(
                {
                    "split": "TrU",
                    "file_type": "image",
...
def prepare_ct_dataset(data_root, nnunet_raw, test=False, num_processes=None):
